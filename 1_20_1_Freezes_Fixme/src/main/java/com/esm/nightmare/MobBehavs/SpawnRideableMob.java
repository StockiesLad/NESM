...
public class SpawnRideableMob {
	...
	void RefreshEntityList() {
		...
		if (RideableList == null) {
			RideableList = com.esm.nightmare.NightmareMain.GetEntitiesFromSerialized(ESMConfig.RideableMobs.get().toString());
			LOGGER.info("RideListLength = " + RideableList.length);
			LOGGER.info("RideList = " + com.esm.nightmare.NightmareMain.EntitiesToSerialList(RideableList));
		}

		if (JockeyList == null) {
			JockeyList = com.esm.nightmare.NightmareMain.GetEntitiesFromSerialized(ESMConfig.JockeyMobs.get().toString());
			LOGGER.info("Jockey List Length = " + JockeyList.length);
			LOGGER.info("JockeyList = " + com.esm.nightmare.NightmareMain.EntitiesToSerialList(JockeyList));
		}
