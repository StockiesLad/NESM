...
public class DuplicateMob {
	...
	void Clone(Mob M) {
		...
		ServerLevel Lvl = (ServerLevel)M.level;
	...
	BlockPos GetOffsetPosition(BlockPos Original_Pos) {
		...
		double Rand_X_Off = new RNG().GetInt(-Swarm_Placement_Proximity, Swarm_Placement_Proximity);
		double Rand_Z_Off = new RNG().GetInt(-Swarm_Placement_Proximity, Swarm_Placement_Proximity);

		// Offset old block position
		BlockPos New_Pos = Original_Pos.offset(Rand_X_Off, 0.0D, Rand_Z_Off);
	...
	boolean isPositionGood(ServerLevel Lvl, BlockPos Position) {
		...
		boolean isTopOK = Lvl.getBlockState(Position.offset(0D, 1D, 0D)).isAir();
